...
def eval_block(*args, &prc)
  if prc.nil?               #see if a proc is given when called
    raise 'NO BLOCK GIVEN!' #raise is similar to p/puts but it shows it as an error message
  else
    prc.call(*args)
  end
end
#how to call
#p eval_block([1,2,3]) {|num| p num} #=> [1,2,3]
