...
class String
  def caesar(shift)
    new_str = ''
    low_case = self.downcase
    letters = ('a'..'z').to_a
    low_case.each_char do |char|
...
class Hash
  def difference(other_hash)
    hash1_keys = self.keys
    hash2_keys = other_hash.keys
    combined = hash1_keys + hash2_keys
    one_timers = combined.select {|el| combined.count(el) == 1}
    new_hash = {}
    self.each do |k,v|
      if one_timers.include?(k)
        new_hash[k] = v
      end
    end
...
class Fixnum
  def stringify(base)
    if base == 16
      ...
      i = 1
      while i <= self
        new_str.unshift(((self/i)%base).to_s)
        i *= base
      end
      str = ''
      new_str.each do |el|
