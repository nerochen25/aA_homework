...
class String
  def caesar(shift)
    new_word_arr = self.each_char.map {|letter| new_word(letter, shift)}
    new_word_arr.join('')
  end

  def new_word(letter, shift)
    all_letters = []
    ('a'..'z').each {|letter| all_letters << letter}
    index_of_letter = all_letters.index(letter)
    shifted_letter = all_letters[(index_of_letter + shift)%26]
    shifted_letter
  end
end
...
class Hash
  def difference(other_hash)
    merged_hash = self.merge(other_hash)
    new_hash = {}
    keys = self.keys
    other_keys = other_hash.keys
    all_keys = keys + other_keys
    unique_keys = all_keys.select {|key| all_keys.count(key) == 1}
    merged_hash.select {|k,v| unique_keys.include?(k) }
  end
end
